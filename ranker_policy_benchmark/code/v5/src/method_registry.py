...
def _validate_registry(registry: MethodRegistry) -> None:
    ...
    if len(registry.rankers) != 2 or {item.ranker_id for item in registry.rankers} != {
        "shil",
        "l1",
    }:
        raise RegistryValidationError("PR-2 requires exactly the shil and l1 rankers")
    if len(registry.methods) != 14:
        raise RegistryValidationError("PR-2 requires exactly 14 MethodSpec records")
