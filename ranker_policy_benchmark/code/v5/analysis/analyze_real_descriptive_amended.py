...
CANDIDATE_SRC = (
    PROJECT_ROOT
    / "experiments"
    / "2026-08-26_codex_vps_track-a-full-candidate"
    / "src"
)
...
def _read_status_csv(path: Path) -> pd.DataFrame:
    """Preserve the protocol's literal N/A status instead of coercing it."""
...
def validate(root: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    sensitivity = _read_status_csv(root / "structured_sensitivity_status.csv")
