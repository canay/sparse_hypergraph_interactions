...
def run_track_a_cell(
    ...
    *,
    ...
    # MCH-SHIL-004 (shil-claude-mch004-v6-ranker-menu-20260903): the v5 grid was
    # 2 rankers x 7 policies = 14.  v6 adds the tree and screen families, so a
    # full-grid cell resolves to 4 x 7 = 28.  The count stays exact and
    # fail-closed; only the declared menu size changed.
    if len(expected_method_ids) != 28:
        raise TrackARunnerError(
            "Track-A full-grid cell must resolve to exactly 28 registry methods"
        )
    methods = tuple(
