"""MCH-SHIL-006: frozen, resumable diagnostic with version-compatible explicit L1."""
from __future__ import annotations
import argparse, copy, ctypes, dataclasses, gzip, hashlib, importlib.metadata
import json, math, os, platform, signal, socket, subprocess, sys, time, uuid, stat
...
def lock():
    paths=[ROOT/'run_extension.py',ROOT/'analyze_extension.py',ROOT/'PROTOCOL.md',
           ROOT/'config/extension.json',ROOT/'config/method_registry.json',ROOT/'vendor_manifest.json',ROOT/'technical_preflight.py',ROOT/'l1_semantic_preflight.py']
    paths+=sorted((ROOT/'vendor').glob('*.py'))
    out={'schema_version':1,'created_at':now(),'change_id':'MCH-SHIL-006',
...
class CachedAdapter:
    ...
    def fit(self,request):
        ...
        if self.ranker_id=='l1' and any('only used when penalty' in str(w) or 'ignored' in str(w).lower() for w in result.warnings):
            raise RuntimeError('L1_PARAMETER_SEMANTICS_WARNING')
...
def worker(args):
    ...
    atomic(folder/'identity.json',{'fingerprint':fingerprint,'dataset_sha256':dataset_hash,'freeze_sha256':freeze_sha,
         ...
         'resolved_l1_parameters':legacy._l1_parameters(0.05,args.seed,2)})
...
def resource_output_bytes(base):
    total=0
...
def supervise(args):
    ...
    if args.phase=='science':
        ...
        semantic=gate['semantic_proofs'][args.host]
        proof_path=(ROOT/semantic['path']).resolve()
        if not proof_path.is_relative_to(ROOT) or sha(proof_path)!=semantic['sha256']:raise RuntimeError('SEMANTIC_PROOF_HASH')
        proof=read(proof_path)
        if proof['status']!='PASS' or proof['freeze_sha256']!=freeze_sha or proof['environment']!=env():raise RuntimeError('SEMANTIC_PROOF_DRIFT')
        if proof['test_sha256']!=sha(ROOT/'l1_semantic_preflight.py') or proof['source_sha256']!=sha(ROOT/'vendor/sc_shil_experiment.py'):raise RuntimeError('SEMANTIC_PROOF_SOURCE')
        if proof['binary_toy_records'][0]['parameters']!=legacy._l1_parameters(0.05,617,2):raise RuntimeError('SEMANTIC_PARAMETER_DRIFT')
    ...
    try:
        while pending or active:
            resources={'available_ram_bytes':psutil.virtual_memory().available,
                       ...
                       'output_bytes':resource_output_bytes(base)}
